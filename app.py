...
def generate_pdf(data, filepath):
    ...
    dims = [("Self-Authority",sa_avg),("Visibility & Voice",vv_avg),
            ("Capability Ownership",co_avg),("Future Self Alignment",fa_avg),
            ("Advancement Behavior",ab_avg),("Decision Patterns",dp_avg),
            ("Advancement Likelihood",al_avg),("Retention Intent",ri_avg)]

    dim_rows = [[Paragraph("Dimension",sTH),Paragraph("Score",sTH),Paragraph("Signal",sTH)]]
    for name, score in dims:
        try: sv = float(score)
        except: sv = 0
        if sv>=70: sig,col="Strong",GREEN
